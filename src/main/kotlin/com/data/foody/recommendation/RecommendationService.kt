...
import com.data.foody.algorithm.SimilarityProcess
...
class RecommendationService(private val dataService: DataService) {
    ...
    suspend fun test(ingredients: String): String {
        val allRecipes = dataService.findAllValuesWithCoroutine()

        // 모든 ingredients_concat 값을 가져옵니다.
        val allIngredients = allRecipes?.map { it.ingredients_concat } ?: listOf()

        // Similarity 객체를 생성합니다.
        val similarity = SimilarityProcess(allIngredients)
        ...
        // 각 레시피에 대해 유사도 계산
        val similarityScores = filteredRecipes?.map { recipe ->
            val score = similarity.tfidfSimilarityUsingClass(ingredients, recipe.ingredients_concat)
            Pair(recipe.recipe_id, score)
        }
